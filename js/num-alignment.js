(function($){ 
	var functions = {
		init:function(input, step, max, min, digit){
			...
			//添加左右加减号
			input.wrap("<div style = 'display:inline-block;margin-left: 40px;margin-top:5px;overflow:hidden;width:" + width + "px;height:" + height + "px;border: 1px solid #CCC;'></div>");
    ...
    $(function(){
		...
		inputs.each(function(){
			//预设值数据选择
			var data = {
			            default_data : 	{"step" : 1, "min" : 0, "max" : 99999, "digit" : 1}, 
			            aaa : 			{"step" : 1, "min" : 0, "max" : 9999, "digit" : 1}, 
						}
			...
			if(digit != null&&!isNaN(parseFloat(digit))){
				digit  =  parseFloat(digit).toFixed();
